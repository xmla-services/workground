...
public class GrabberServiceImpl implements GrabberService {
    ...
    private void grabData(String endPointUrl) {
        ...
        regularLevels.forEach(it -> {
            ...
            executeDictionaryQueryTargetDatabaseQuery(statementResponse, 2 + columns.size(), sqlQuery);
        });
    }
    ...
    private void executeDictionaryQueryTargetDatabaseQuery(
    ...
    private void batchExecute(PreparedStatement ps, int portion, List<CellType> cell) throws SQLException {
        ...
        for (List<CellType> cells : subSets) {
            for (int i = 0; i < cells.size(); i++) {
                ps.setString(i, cells.get(i).value().toString());
                //TODO
            }
            ps.addBatch();
            count++;
