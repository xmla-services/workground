...
class SelectSubCubeClauseTest {
	...
	class SelectSubCubeClauseStatementTest {
		...
		void testSingleSubCube() throws MdxParserException {
			...
			assertThat(axis.named()).isTrue();
			assertThat(axis.ordinal()).isZero();
			...
			assertThat(keyObjectIdentifier.nameObjectIdentifiers().get(0).quoting())
					.isEqualTo(ObjectIdentifier.Quoting.QUOTED);
	...
	void testMultiSubCube() throws MdxParserException {
		...
		assertThat(axisInner.named()).isTrue();
		assertThat(axisInner.ordinal()).isZero();
		...
		NameObjectIdentifier nameObjectIdentifierInner = (NameObjectIdentifier) compoundIdInner.objectIdentifiers()
				.get(0);
