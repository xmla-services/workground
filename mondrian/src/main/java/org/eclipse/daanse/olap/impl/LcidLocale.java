...
public class LcidLocale {
    final Map<Short, LcidLocale.Info> lcidLocaleMap = new HashMap<Short, LcidLocale.Info>();
    final Map<String, LcidLocale.Info> localeToLcidMap = new HashMap<String, LcidLocale.Info>();
    ...
    private Locale toLocale(short lcid) {
        final LcidLocale.Info s = lcidLocaleMap.get(lcid);
        if (s == null) {
    ...
    private short toLcid(String localeName) {
        ...
        for (;;) {
            final LcidLocale.Info info = localeToLcidMap.get(localeName);
            if (info != null) {
                return info.lcid;
            }
