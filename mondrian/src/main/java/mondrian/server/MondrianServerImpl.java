 ...
 * @since Jun 25, 2006
 */
public class MondrianServerImpl extends MondrianServer {
	/**
	 ...
	 * construction, and are removed when they call close. Garbage collection may
	 * cause a connection to be removed earlier.
	 */
	...
	/**
	 * Map of open statements, by id. Statements are added just after construction,
	 * and are removed when they call close. Garbage collection may cause a
	 * connection to be removed earlier.
	 */
	@SuppressWarnings("unchecked")
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(MondrianServerImpl.class);
	 ...
	 */
	public MondrianServerImpl(Context context) {
		this.context = context;
		this.aggMgr = new AggregationManager(context);
	...
	synchronized public void addConnection(RolapConnection connection) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("addConnection , id={}, statements={}, connections=", id, getStatementMap().size(),
					connectionMap.size());
		}
		if (shutdown) {
			throw new MondrianException("Server already shutdown.");
	...
	synchronized public void removeConnection(RolapConnection connection) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("removeConnection , id={}, statements={}, connections={}", id, getStatementMap().size(),
					connectionMap.size());
		}
		if (shutdown) {
			throw new MondrianException("Server already shutdown.");
		...
		monitor.sendEvent(new ConnectionEndEvent(System.currentTimeMillis(), context.getName(), connection.getId()));
	}
	...
	synchronized public void addStatement(Statement statement) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("addStatement , id={}, statements={}, connections={}", id, getStatementMap().size(),
					connectionMap.size());
		}
		getStatementMap().put(statement.getId(), statement);
	...
	synchronized public void removeStatement(Statement statement) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("removeStatement , id={}, statements={}, connections={}", id, getStatementMap().size(),
					connectionMap.size());
		}
		if (shutdown) {
			throw new MondrianException("Server already shutdown.");
		}
		getStatementMap().remove(statement.getId());
		final RolapConnection connection = statement.getMondrianConnection();
