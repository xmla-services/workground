...
@Designate(ocd = BasicVerifierConfig.class)
@Component(service = Verifyer.class)
public class BasicVerifyer implements Verifyer {

    private static final Logger LOGGER = LoggerFactory.getLogger(BasicVerifyer.class);
    public static final Converter CONVERTER = Converters.standardConverter();

    private BasicVerifierConfig config;

    @Activate
    public void activate(Map<String, Object> configMap) {
        this.config = CONVERTER.convert(configMap)
                .to(BasicVerifierConfig.class);
    }
    ...
    public List<VerificationResult> verify(Schema schema, DataSource dataSource) {
        ...
        return new BasicSchemaWalker(config).checkSchema(schema);
    }
