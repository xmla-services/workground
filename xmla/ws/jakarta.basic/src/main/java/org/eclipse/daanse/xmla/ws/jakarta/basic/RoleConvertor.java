...
public class RoleConvertor {
    ...
    public static Role convertRole(org.eclipse.daanse.xmla.ws.jakarta.model.xmla.xmla.Role role) {
        if (role != null) {
            return new RoleR(role.getName(),
                role.getID(),
                convertToInstant(role.getCreatedTimestamp()),
                convertToInstant(role.getLastSchemaUpdate()),
                role.getDescription(),
                convertAnnotationList(role.getAnnotations() == null ? null : role.getAnnotations().getAnnotation()),
                convertMemberList(role.getMembers() == null ? null : role.getMembers().getMember()));
        }
        return null;
    }
    ...
    private static Member convertMember(org.eclipse.daanse.xmla.ws.jakarta.model.xmla.xmla.Member member) {
        if (member != null) {
            return new MemberR(member.getName(),
                member.getSid());
        }
        return null;
    }
